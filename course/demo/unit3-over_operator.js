...
function fillScene() {
  ...
  sourceMaterial = new THREE.MeshLambertMaterial({
    color: 0xe53319,
    opacity: 0.7,
    transparent: true,
  })
  ...
  // block
  var cube = new THREE.Mesh(new THREE.CubeGeometry(10, 50, 150), destMaterial)
  cube.position.x = 30
  scene.add(cube)

  cube = new THREE.Mesh(new THREE.CubeGeometry(10, 150, 50), sourceMaterial)
  cube.position.x = -30
  scene.add(cube)
}
...
function render() {
  ...
  sourceMaterial.color.setRGB(
    effectController.sred,
    effectController.sgreen,
    effectController.sblue
  )
