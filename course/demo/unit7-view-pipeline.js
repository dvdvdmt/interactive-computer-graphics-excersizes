...
function init() {
  ...
  camera = new THREE.PerspectiveCamera(
    effectController.fov,
    aspect,
    effectController.near,
    effectController.far
  )
...
function createGridScene() {
  ...
  // CAMERA
  cameraGrid = new THREE.OrthographicCamera(
    -0.5 * canvasWidth,
    0.5 * canvasWidth,
    -0.5 * canvasHeight,
    0.5 * canvasHeight,
  ...
  for (var mchunk = 0; mchunk < messageList.length; mchunk++) {
    ...
    if (showRect && dofill) {
      ...
      roundRect(
        ...
        borderThickness / 2,
        borderThickness / 2,
        textWidth + borderThickness + 2 * EXTRA_CUSHION,
        fontsize * (1.2 * lines + 0.2) + borderThickness + 2 * EXTRA_CUSHION,
        6,
        fill
      )
    }
    ...
    for (var style = 0; style < 2; style++) {
      ...
      for (i = 0; i < cleanStringList.length; i++) {
        metrics = context.measureText(cleanStringList[i])
        context.fillText(
          style ? highlightStringList[i] : normalStringList[i],
          offsetx +
            borderThickness +
            EXTRA_CUSHION +
            (textAlignment == 'right' ? textWidth - metrics.width : 0),
...
function createHelp() {
  var message = [
    'This demo shows how a single vertex',
    'of a box, marked with a light blue dot,',
    'is transformed by the rendering pipeline.',
    'Red numbers are those that change.',
    'Each transform is shown in OpenGL/WebGL',
    'order: the untransformed vector is',
    'on the right, the matrix multiplies it,',
    'and the resulting vector is on the left.',
  ...
  helpSprite.position.set(canvasWidth - 0.7 * canvasHeight, canvasHeight, 0.5)
...
function createText(force) {
  ...
  if (!force) {
    ...
    if (
      matrixMatch(prevMatrixWorld, cube.matrixWorld) &&
      prevPtm.equals(ptm) &&
    ...
    switch (displayList[modenum]) {
      ...
      case 'model':
        ...
        for (c = 0; c < 4; c++) {
          for (r = 0; r < 4; r++) {
            setHighlights(
              cube.matrixWorld.elements[r * 4 + c],
              prevMatrixWorld.elements[r * 4 + c],
              hl
            )
            messageList[i] += sprintf(
              '%s%6.2f%s',
      ...
      case 'view':
        ...
        for (c = 0; c < 4; c++) {
          for (r = 0; r < 4; r++) {
            setHighlights(
              camera.matrixWorldInverse.elements[r * 4 + c],
              prevMatrixWorldInverse.elements[r * 4 + c],
              hl
            )
            messageList[i] += sprintf(
              '%s%7.2f%s',
      ...
      case 'projection':
        ...
        for (c = 0; c < 4; c++) {
          ...
          setHighlights(
            camera.projectionMatrix.elements[4 + c],
            prevProjectionMatrix.elements[4 + c],
            hl
          )
          messageList[i] += sprintf(
            '%s%6.2f%s',
          ...
          setHighlights(
            camera.projectionMatrix.elements[8 + c],
            prevProjectionMatrix.elements[8 + c],
            hl
          )
          messageList[i] += sprintf(
            '%s%7.2f%s',
          ...
          setHighlights(
            camera.projectionMatrix.elements[12 + c],
            prevProjectionMatrix.elements[12 + c],
            hl
          )
          messageList[i] += sprintf(
            '%s%7.2f%s ',
...
function createFrustum(pointsForDepth, faces, refresh) {
  ...
  for (x = 0; x <= 1; x++) {
    for (y = 0; y <= 1; y++) {
      for (z = 0; z < pointsForDepth; z++) {
        ...
        frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + z] = new THREE.Vector3(
          world.x,
          world.y,
          world.z
        )
  ...
  for (x = 0; x <= 1; x++) {
    for (y = 0; y <= 1; y++) {
      if (refresh) {
        ...
        lineGeometry[gcount].vertices.push(
          frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + (pointsForDepth - 1)]
        )
  ...
  for (z = 0; z < pointsForDepth; z++) {
    if (refresh) {
      ...
      for (v = 0; v < 5; v++) {
        ...
        lineGeometry[gcount].vertices.push(
          frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + z]
        )
      }
      mtl = lineMaterial[z]
  ...
  if (refresh) {
    ...
    for (v = 0; v < 4; v++) {
      x = Math.floor(v / 2) % 2
      y = Math.floor((v + 1) / 2) % 2
      depthFaceGeometry[0].vertices[v].copy(
        frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth]
      )
    }
    ...
    if (refresh) {
      for (z = 1; z < pointsForDepth; z++) {
        ...
        for (v = 0; v < 4; v++) {
          x = Math.floor(v / 2) % 2
          y = Math.floor((v + 1) / 2) % 2
          depthFaceGeometry[z].vertices.push(
            frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + z]
          )
        }
      ...
      for (z = 1; z < pointsForDepth; z++) {
        ...
        for (v = 0; v < 4; v++) {
          x = Math.floor(v / 2) % 2
          y = Math.floor((v + 1) / 2) % 2
          depthFaceGeometry[z].vertices[vcount++].copy(
            frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + z]
          )
        }
    ...
    if (refresh) {
      for (side = 0; side < 4; side++) {
        ...
        for (v = 0; v < 4; v++) {
          ...
          sideFaceGeometry[side].vertices.push(
            frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + z * (pointsForDepth - 1)]
          )
        }
      ...
      for (side = 0; side < 4; side++) {
        ...
        for (v = 0; v < 4; v++) {
          ...
          y = Math.floor((side * 4 + v + 2) / 8) % 2
          z = Math.floor((v + 1) / 2) % 2
          sideFaceGeometry[side].vertices[vcount++].copy(
            frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + z * (pointsForDepth - 1)]
          )
        }
  ...
  if (refresh) {
    ...
    for (v = 0; v < 4; v++) {
      x = Math.floor(v / 2) % 2
      y = Math.floor((v + 1) / 2) % 2
      sideFaceGeometry[4].vertices.push(
        frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + (pointsForDepth - 1)]
      )
    }
    ...
    for (v = 0; v < 4; v++) {
      x = Math.floor(v / 2) % 2
      y = Math.floor((v + 1) / 2) % 2
      sideFaceGeometry[4].vertices[vcount++].copy(
        frustumPoints[x * 2 * pointsForDepth + y * pointsForDepth + (pointsForDepth - 1)]
      )
    }
...
function setupGui() {
  ...
  var gui = new dat.GUI()
  gui
    .add(effectController, 'matrix', ['model', 'view', 'projection', 'window', 'all'])
    .name('Watch matrix')
  ...
  gui
    .add(effectController, 'viewport', ['off', 'volume', 'near/far', 'depths'])
    .name('Show frustum')
...
function render() {
  ...
  if (effectController.viewport != 'off') {
    ...
    renderer.setScissor(
      (1.0 - margin - viewsize - borderh) * canvasWidth,
      margin * canvasHeight,
      (viewsize + borderh) * canvasWidth,
      (viewsize + borderv) * canvasHeight
    )
    ...
    renderer.setClearColorHex(clearColor, 1.0)
    renderer.setScissor(
      (1.0 - margin - viewsize - borderh / 2) * canvasWidth,
      (margin + borderv / 2) * canvasHeight,
      viewsize * canvasWidth,
      viewsize * canvasHeight
    )
    renderer.setViewport(
      (1.0 - margin - viewsize - borderh / 2) * canvasWidth,
      (margin + borderv / 2) * canvasHeight,
    ...
    createFrustum(
      effectController.viewport == 'depths' ? 5 : 2,
