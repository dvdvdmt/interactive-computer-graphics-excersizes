...
function init() {
  ...
  darkamTexture.wrapS = THREE.RepeatWrapping
  darkamTexture.wrapT = THREE.RepeatWrapping

  // MATERIALS
  material.crate = new THREE.MeshBasicMaterial({map: crateTexture})
  material.grid = new THREE.MeshBasicMaterial({map: gridTexture})
  material.maple = new THREE.MeshBasicMaterial({
    map: mapleTexture,
    transparent: true,
  })
  material.feather = new THREE.MeshBasicMaterial({
    map: featherTexture,
    transparent: true,
  })
...
function setupGui() {
  ...
  gui
    .add(effectController, 'mtlName', ['crate', 'grid', 'maple', 'feather', 'darkam'])
    .name('texture image')
