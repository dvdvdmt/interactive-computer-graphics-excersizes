...
function createStairs() {
  ...
  const riserHeight = verticalStepHeight + stepThickness
  var stepMesh
  ...
  for (let i = 0; i <= 5; i++) {
    ...
    stepMesh.position.y = i * riserHeight + verticalStepHeight / 2 // half of height: put it above ground plane
    stepMesh.position.z = i * riserDepth // centered at origin
    scene.add(stepMesh)
    ...
    stepMesh.position.y = stepThickness / 2 + verticalStepHeight + i * riserHeight
    // Push step forward by half the depth, minus half the vertical step's thickness
    stepMesh.position.z = i * riserDepth + horizontalStepDepth / 2 - stepHalfThickness
    scene.add(stepMesh)
  }
}
